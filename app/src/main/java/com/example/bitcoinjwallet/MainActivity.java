...
public class MainActivity extends Activity {
                ...
                askTwoPasswords("Set wallet password", (p1, p2) -> {
                    ...
                    try {
                        KeyCrypterScrypt scrypt = new KeyCrypterScrypt(
                                org.bitcoinj.crypto.KeyCrypter.ScryptParameters.withP(6));
            ...
            if (seed.isEncrypted()) {
                askPassword("Wallet password", pass -> {
                    try {
                        ...
                        showSeedText(clear.toString());
                    } catch (Throwable t) {
                        Toast.makeText(this, message(t), Toast.LENGTH_LONG).show();
                    }
                });
            } else {
                showSeedText(seed.toString());
            }
        } catch (Throwable t) {
            Toast.makeText(this, message(t), Toast.LENGTH_LONG).show();
        }
    ...
    private void showTool() {
        ...
        run.setOnClickListener(v -> {
            ...
            if (!chain.getText().toString().trim().isEmpty()) add(a, "--chain", chain.getText().toString().trim());
